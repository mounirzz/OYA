<%@ page language="java" contentType="text/html; charset=UTF-8"
         pageEncoding="UTF-8" %>
<!DOCTYPE html>
<html>
<head>
    <%@ include file="/common/global.jsp" %>
    <title>Gestion des produits</title>
    <%@ include file="/common/meta.jsp" %>
    <%@ include file="/common/include-base-styles.jsp" %>
    <link type="text/css" rel="stylesheet" href="${ctx}/css/product.css"/>
    <%@ include file="/common/include-base-js.jsp" %>
    <script src="${ctx }/js/product.js" type="text/javascript"></script>
</head>

<body>
<%@include file="/common/admin-admin-nav.jsp" %>
<div class="container">
 
    <div class="row">
        <table class="table table-striped">
            <thead>
                <tr>
            	     <td> ID </td>
                     <td> nom </td>
                     <td> modèle </td>
                     <td> code </td>
                     <td> prix </td>
                     <td> Inventaire </td>
                     <td> opération </td>
                </tr>
            </thead>
            <tbody>
                <c:forEach items="${page.result}" var="product">
                    <tr>
                        <td>${product.id}</td>
                        <td><a href="${ctx}/product/${id}">${product.title}</a></td>
                        <td>${product.model}</td>
                        <td>${product.code}</td>
                        <td>${product.carburant}</td>
                        <td>${product.model}</td>
                        <td>${product.point}</td>
                        <td>${product.stock}</td>
                        <td><a class="button" href="${ctx}/admin/product/edit/${product.id}">Modifier</a></td>
                    </tr>
                </c:forEach>
            </tbody>
        </table>
    </div>
    <div class="text-center">
        <tags:pagination page="${page}" paginationSize="${page.pageSize}"/>
    </div>
    <a class="button" href="${ctx}/admin/product/new">Ajouter</a>

</div>
<!-- /container -->
<%@include file="/common/footer.jsp" %>
</div>
<!-- Dashboard / End -->


</div>
<!-- Wrapper / End -->
</body>
</html>