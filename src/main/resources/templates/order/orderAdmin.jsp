...
                     <td> opération </td>
                ...
                <c:forEach items="${page.result}" var="order">
                    <tr pid="${order.id}">
                        ...
                        <td>
                            <c:choose>
                                <c:when test="${order.status==0}"><a class="btn btn-info btn-xs" href="${ctx}/order/view/${order.id}">Pour voir</a> <a class="btn btn-info btn-xs del-order-btn">Supprimé</a></c:when>
                                <c:when test="${order.status==1}"><a class="btn btn-info btn-xs" href="${ctx}/order/view/${order.id}">Pour voir</a> <a class="btn btn-info btn-xs ship-order-btn">Livraison</a> <a class="btn btn-info btn-xs del-order-btn">Supprimé</a></c:when>
                                <c:when test="${order.status==2}"><a class="btn btn-info btn-xs" href="${ctx}/order/view/${order.id}">Pour voir</a></c:when>
